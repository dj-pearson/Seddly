...
<#
.SYNOPSIS
    Ralph — Autonomous build loop for Seddly using Claude Code CLI.
.DESCRIPTION
...
Write-Host "  RALPH - Seddly Autonomous Build Loop" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    while ($iteration -lt $MaxIterations) {
        ...
        $storyCategory = if ($story.category) { $story.category } else { "General" }
        $remaining = Get-RemainingCount
        ...
        Write-Host " [$storyCategory] " -NoNewline -ForegroundColor DarkYellow
